    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                y = x[i].sigmoid()
                dim = len(list(y.size()))-1
                
                # y[..., 0:2] = (y[..., 0:2] * 2. - 0.5 + self.grid[i].to(x[i].device)) * self.stride[i]  # xy
                self.grid[i] = self.grid[i].cpu().numpy()
                self.grid[i] = torch.tensor(self.grid[i]).to(x[i].device)
                mv_y = (y[..., 0:2] * 2. - 0.5 + self.grid[i]) * self.stride[i]
                y = torch.cat((mv_y, y[..., 2:]), dim=dim)
                # y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh
                y = torch.cat((y[..., 0:2], (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i], y[..., 4:]), dim=dim)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='models/yolov5s_deconv.yaml', help='model.yaml')
    ...
    torch.onnx.export(model,               # model being run
                      ...
                      "s2.onnx",   # where to save the model (can be a file or file-like object)
                      opset_version=11,
